...
class GelloPublisher(Node):
    ...
    def __init__(self) -> None:
        ...
        # 夹爪"按住(momentary)"控制：扳机按住期间夹住，松开立刻张开（不再 toggle 翻转保持）。
        # 下面是去抖阈值与二值输出，可按手感用 ROS 参数调。
        # 注意 GELLO 主手扳机 percent：低=捏紧、高=松开（与下游 input_closed_value 一致）。
        # 扳机 percent 低=捏紧、高=松开，静止(完全松开)约 1.0、捏到底约 0.24。
        # press_threshold 越高 -> 需要的捏合行程越小、越省力。0.8 表示 percent 只要从
        # 1.0 轻轻掉到 0.8（约 1/4 行程，即"稍微按下"）就判定为按住 -> 夹住，不必"按死"。
        # 保持迟滞(press < release)只为消除临界抖动：跌破 press 夹住，回升过 release 张开，
        # 两阈值之间为死区保持当前状态——不是 toggle，按住一直闭合、松开即张开。
        self._gripper_press_threshold = (
            self.declare_parameter("gripper_press_threshold", 0.8)
            .get_parameter_value()
            .double_value
        )
        self._gripper_release_threshold = (
            self.declare_parameter("gripper_release_threshold", 0.9)
            .get_parameter_value()
            .double_value
        )
        # 发布的二值 percent：夹住发 closed_output，张开发 open_output。
        # 下游 gripper_client 会把它离散成 grasp/move，录制端会把它二值化进数据集。
        self._gripper_closed_output = (
            self.declare_parameter("gripper_closed_output", 0.0)
            .get_parameter_value()
            .double_value
        )
        self._gripper_open_output = (
            self.declare_parameter("gripper_open_output", 1.0)
            .get_parameter_value()
            .double_value
        )
        # 初始张开。按住模式按当前扳机位置实时判定，无需 toggle 的"开机点击去抖"状态。
    ...
    def publish_joint_jog(self) -> None:
        ...
        gripper_joint_states.data = self._update_gripper_hold(gripper_position)
        self.arm_joint_publisher.publish(arm_joint_states)
        self.gripper_joint_publisher.publish(gripper_joint_states)

    def _update_gripper_hold(self, gripper_percent: float) -> float:
        """按住(momentary)模式：扳机按住期间夹住，松开立刻张开。

        返回二值 percent（夹住 -> closed_output，张开 -> open_output）。

        扳机 percent 低=捏紧、高=松开。用迟滞去抖消除临界抖动：percent 跌破
        press_threshold 即判定"按下" -> 夹住；回升过 release_threshold 判定"松开"
        -> 张开；两阈值之间为死区，保持当前状态。与旧的 toggle 不同：这里随扳机
        实时跟随——按住一直闭合、一松开就张开，不再翻转保持。
        """
        if gripper_percent < self._gripper_press_threshold:
            self._gripper_closed = True
        elif gripper_percent > self._gripper_release_threshold:
            self._gripper_closed = False

        return (
            self._gripper_closed_output
            if self._gripper_closed
            else self._gripper_open_output
        )
