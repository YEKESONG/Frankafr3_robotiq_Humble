...
任何一行返回 `reply (...)` 就说明夹爪在总线上。**全部 `no reply` 说明问题在
ROS 之下**：多半是 24V 没供电（USB-RS485 适配器自己吃 USB 电，夹爪没电时照样枚举出
`/dev/ttyUSB*`，很容易误判成"接好了"），也可能是 RS-485 的 A/B 接反。
...
用 `scripts/probe_robotiq_serial.py` 绕开 ROS 复测，
5 种波特率 × 2 个从站地址**全部 no reply** —— 说明夹爪没有出现在总线上，
问题在 ROS 之下。最可能是 **24V 没供电**，其次是 RS-485 的 A/B 接反。
供上电之后重跑上面的 ② 即可确认；ROS 侧不需要再改。
